...
#
[string] $SqlQuery= $("SELECT * FROM [DataExchangeDB].[MIMS].[Persons_FULL_and_uvol]  where [company] like '%МФ ОЦО ПАО `"Ростелеком`"%'`
and ([extensionAttribute11] like '%г. Москва%' or [extensionAttribute11] like '%Московская обл.%' )` `
...
#[string] $SqlQuery= $("SELECT * FROM [DataExchangeDB].[MIMS].[Persons_FULL_and_uvol]  where [company] like '%Корпоративный центр%' and [extensionAttribute9] like '%основное место работы%' and ([require_login] like '%Y%' or [lev_grade] ='2' or [lev_grade] ='3')")
#[string] $SqlQuery= $("SELECT * FROM [DataExchangeDB].[MIMS].[Persons_FULL_and_uvol]  where [company] like '%Корпоративный центр%' and [extensionAttribute9] like '%основное место работы%' and [require_login] like '%Y%'")
...
 
#$DataSet.Tables[0] | out-file "C:\temp2\NewOCO.csv"
$PayloadData = $DataSet.Tables[0]

# EO get data from DB
#---------------------------------------------------------------------
#Variable Set
$ConnectionExchange = 'http://SGD02EX010.gd.rt.ru/PowerShell'
$GC = 'sgd02dc001.gd.rt.ru:3268'
$DC = 'sgd02dc001.gd.rt.ru'
$dbname= 'GD02 MBStore02 SGD02EXN02' |Get-Random
$OU = 'OU=MRF_OCO,OU=USERS&PC,OU=GD,DC=GD,DC=RT,DC=RU'
$DestPathLogOCO = '\\Mt.rt.ru\public\IT\New accounts\Logs\OCO'
$tempOCOfolder = 'C:\temp\oco\'
        ...
        IF (($Employeer.ASSIGNMENT_START_DATE).ToString("yyyy-MM-dd") -ge (Get-Date).AddDays(-5).ToString("yyyy-MM-dd"))
            ...
            $Exist_AD_Account = get-aduser -Properties EmployeeID -filter {EmployeeID -eq $ID} -Server $GC 
                IF ($Exist_AD_Account) 
                    ...
                    $exist|out-file "$tempOCOfolder\Exist_OCO_Users_$date.csv" -Append utf8
                    }
                Else
                    {
                     ...
                     while (get-aduser -Properties userprincipalname -Filter {SamAccountName -eq $SAMAcc} -Server $GC)
                             ...
                             #$k++;}
                              ...
                              #}             
                                ...
                                try {
                                    New-ADUser @attr_newaduser -Server $DC -verbose -ea stop;Start-Sleep -Seconds 7
                                   
                                    }
                                catch
                                    {
                                    "Error caught $($Error[0].Exception)" |out-file $ErrorFolder\NotCreateMoscowOCOAccount-$SAMAcc-$date.txt
                                    Set-ADUser -identity "CN=$Name,$OU" -Add @{EmployeeID=$ID;EmployeeType=$EmployeeType} -Server $DC;
                                    ...
                                    $set| out-file "$tempOCOfolder\set $name $date.csv" -Append utf8
                                    }
                                 Set-ADUser -identity $SAMAcc -Add @{extensionAttribute8="New Script for future Employeers"} -Server $DC -ea Continue;
                                 Set-ADUser -identity $SAMAcc -Add @{EmployeeType=$EmployeeType} -Server $DC -ea Continue;
                                 #Set-ADUser -identity $SAMAcc -Add @{extensionAttribute9="$($Account_owner)"} -Server $DC;
                                    
                                 try {
                                      Enable-Mailbox -Identity $SAMAcc -Database $dbname -Alias $SAMAcc -DomainController $DC -Force
                                      }
                                catch {
                                    "Error caught $($Error[0].Exception)" |out-file $ErrorFolder\NotCreateMoscowOCOMailbox-$SAMAcc-$date.txt -Append
                                    }
                                 ...
                                 #$Create |out-file "$DestPathLogOCO\$name $date.csv" -Append utf8
                                ...
                                $info.Адрес = $Address; 
                                $info | out-file "$DestPathLogOCO\$name $date.txt" utf8 
...
Remove-PSSession *
$OCOOU = 'OU=MRF_OCO,OU=USERS&PC,OU=GD,DC=GD,DC=RT,DC=RU'
$GDOU = 'OU=GD_USERS,OU=USERS&PC,OU=GD,DC=GD,DC=RT,DC=RU'
$RRSOU = 'OU=Users,OU=MSK,OU=RRS,OU=USERS&PC,OU=GD,DC=GD,DC=RT,DC=RU'
$RTIOU = 'OU=RT_Integration,OU=USERS&PC,OU=GD,DC=GD,DC=RT,DC=RU'
...
foreach ($acc in $NewAccounts)
        {
        if ($acc.company -match "Корпоративный центр ПАО `"Ростелеком`"") {Move-ADObject -Identity $acc -TargetPath $GDOU -Server $DC -Confirm:$false}
        elseif ($acc.company -match "ООО `"Ростелеком - Розничные системы`"") {Move-ADObject -Identity $acc -TargetPath $RRSOU -Server $DC -Confirm:$false}
        elseif ($acc.company -match "ООО `"Ростелеком-Интеграция`"") {Move-ADObject -Identity $acc -TargetPath $RTIOU -Server $DC -Confirm:$false}
        elseif ($acc.company -match "МФ ОЦО ПАО `"Ростелеком`"") {Move-ADObject -Identity $acc -TargetPath $OCOOU -Server $DC -Confirm:$false}
        }
