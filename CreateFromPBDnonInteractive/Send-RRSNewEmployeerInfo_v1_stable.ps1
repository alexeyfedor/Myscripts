...
# добавлен файл подстановки e-mail адресов руководителей
...
and  [ASSIGNMENT_START_DATE] = DATEADD(day, DATEDIFF(day, 0, getdate()), 0) `
...
foreach ($EmployeerRRS in $PayloadData)
    {
        ...
        IF (($EmployeerRRS.ASSIGNMENT_START_DATE).ToString("yyyy-MM-dd") -eq (Get-Date).ToString("yyyy-MM-dd"))
            {
            ...
            try {
                ...
                #create set of attrubutes
                     $Nameforexception = $EmployeerRRS.CN
                     ...
                     $Hired_date_for_Email = ($EmployeerRRS.ASSIGNMENT_START_DATE).ToString("dd.MM")
                     $Chief = $EmployeerRRS.manager
                     $ChiefIO = ([string](($EmployeerRRS.manager -split " ")[1,2]))
                     ...
                     $ChiefMail = [string]$EmployeerRRS.mail_chif
                ...
                $info  | ConvertTo-Html -as List -Head $Header |Out-File "$DestFolderHTML\$name.htm" -force
...
"@ 
            ...
            {
            "$($Error[0]) User $Nameforexception  with EmployeeID $ID something went wrong. Check html output in $DestFolderHTML and set-aduser block, in script " |out-file "$DestFolderError\Send-$Nameforexception.txt" -Force utf8
            }
         
        }#EO IF
        #Else {write-host "User $($EmployeerRRS.CN) starting work not today" -ForegroundColor Yellow}

}#EO Foreach

#check email delivery
#'sce09ex01','sce09ex02','sce09ex03','sce09ex04','sce09ex05','sce09ex06' |Get-MessageTrackingLog -Sender 'NewEmployeerScript@rt.ru' -Start (get-date).AddDays(-1)
