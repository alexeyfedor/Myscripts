...
 
#$DataSet.Tables[0] | out-file "C:\temp2\telegraph.csv"
...
$OU = 'OU=Users,OU=Migrated,OU=DR,OU=MRF Center,DC=ce,DC=RT,DC=RU'
$DestPathLogsMRFC = 'c:\temp\UsersFromTelegraph'
$tempfolder = 'C:\temp'
...
foreach ($Employeer in $PayloadData)
    {
        ...
        $DisplayName = $Employeer.DisplayName
        IF (($Employeer.ASSIGNMENT_START_DATE).ToString("yyyy-MM-dd") -ge (Get-Date).AddDays(-45).ToString("yyyy-MM-dd"))
            {
            
            $Exist_AD_Account = get-aduser -Properties EmployeeID -filter {EmployeeID -eq $ID} -Server $GC 
                IF ($Exist_AD_Account) 
                    {
                    $exist = "User $($Exist_AD_Account.distinguishedName) with EmployeeID $ID exist in forest RT.RU. Check existing accounts in forest RT.RU" 
                    $exist|out-file "$tempfolder\Exist_MRFC_Telegrath_Users_$date.csv" utf8 -Append
                    }
                Else
                    {
                    $check  = get-aduser -SearchBase $OU -filter {Name -eq $DisplayName} -Server $DC 
                        If ($check) 
                            {
                            #write-host "User $DisplayName exist in OU=Migrate"
                            Set-ADUser -Identity $check -Add @{EmployeeID=$ID} -Server $DC  *>> "C:\WorkSet\Telegraph\$($check.distinguishedName).txt";
                            Set-ADUser -identity $check -Add @{EmployeeType=$EmployeeType} -Server $DC 
                            Enable-ADAccount -Identity $check -Confirm:$false
                             $Checkresult = "УЗ работника Центрального Телеграфа $($check.distinguishedName) прописаны EmployeeID $($ID) и EmployeeType $($EmployeeType) " 
                             $Checkresult |out-file "$DestPathLogsMRFC\$DisplayName $date.csv"  utf8
