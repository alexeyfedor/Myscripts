...
[string] $SqlQuery= $("SELECT * FROM [DataExchangeDB].[MIMS].[Persons_FULL_and_uvol]  where [company] like '%Корпоративный центр%'`
...
 
#$DataSet.Tables[0] | out-file "C:\temp2\NewKC.csv"
...
$DestPathLogsKC = '\\Mt.rt.ru\public\IT\New accounts\Logs\KC'
$tempfolder = 'C:\temp'
        ...
        IF (($Employeer.ASSIGNMENT_START_DATE).ToString("yyyy-MM-dd") -ge (Get-Date).AddDays(-5).ToString("yyyy-MM-dd"))
            ...
            $Exist_AD_Account = get-aduser -Properties EmployeeID -filter {EmployeeID -eq $ID} -Server $GC 
                IF ($Exist_AD_Account) 
                    ...
                    $exist|out-file "$tempfolder\Exist_KC_Users_$date.csv" -Append utf8
                    }
                Else
                    {
                        ...
                        #while (get-aduser -Properties userprincipalname -Filter {userprincipalname -eq $UPN} -Server $GC)
                             ...
                             #>
                                ...
                                try {
                                    ...
                                    {
                                    "Error caught $($Error[0].Exception)" |out-file C:\WorkSet\Error\NotCreateKCAccount_$SAMAcc-$date.txt
                                 ...
                                 try {
                                    ...
                                    "Error caught $($Error[0].Exception)" |out-file $ErrorFolder\NotCreateKCMailbox-$SAMAcc-$date.txt -Append
                                    } 
                                 ...
                                 #$Create |out-file "$DestPathLogsKC\$name $date.csv" -Append utf8
                                ...
                                $info.Адрес = $Address; 
                                $info | out-file "$DestPathLogsKC\$name $date.txt" utf8 
