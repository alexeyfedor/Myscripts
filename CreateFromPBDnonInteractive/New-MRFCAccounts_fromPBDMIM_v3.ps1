                        ...
                        #while (get-aduser -Properties userprincipalname -Filter {userprincipalname -eq $UPN} -Server $GC)
                              {
                                ...
                                # перемещение УЗ в региональное OU
                                    ...
                                    "*Липецк*" {$DN=(Get-ADUser $SAMAcc).DistinguishedName;Move-ADObject -Identity $DN -TargetPath 'OU=Users,OU=LP,OU=MRF Center,DC=ce,DC=RT,DC=RU' -Server $DC -Confirm:$false;$DestPathLogsMRFC ='\\Mt.rt.ru\public\IT\New accounts\Logs\MRFC\LP'}
