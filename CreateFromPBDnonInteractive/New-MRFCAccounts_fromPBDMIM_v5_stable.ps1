...
<#
v3 добавлено перемещение УЗ  пользователя Дирекции (по орг. структуре) в зависимости от территориального признака в OU филиалов
v3 добавлены атрибуты EmployeeNumber и ExtensionAttribute11  при создании УЗ
v3 добавлен вывод информации о созданных УЗ в виде таблицы
v3 добавлен поиск по атрибуту ASSIGNMENT_START_DATE вместо Hire_date
v5 изменена генерация SamAccountName
v5 добавлен вывод исключений при создании УЗ в файл в папке C:\WorkSet\Error
#>
...
[string] $SqlQuery= $("SELECT * FROM [DataExchangeDB].[MIMS].[Persons_FULL_and_uvol]  where [company] like '%МРФ `"Центр`" ПАО `"Ростелеком`"%' `
and [extensionAttribute9] like '%основное место работы%' and ([require_login] like '%Y%' or [lev_grade] ='2' or [lev_grade] ='3')`
and [ASSIGNMENT_START_DATE] >= DATEADD(DAY,-10,CURRENT_TIMESTAMP)")
...
 
#$DataSet.Tables[0] | out-file "C:\temp2\MRFC.csv"
...
$dbname= 'ce-min-001','ce-min-002','ce-min-003','ce-min-004','ce-min-005','ce-min-006','ce-min-007','ce-min-008','ce-min-009','ce-min-010','ce-min-011','ce-min-012','ce-min-013','ce-min-014' |Get-Random
...
foreach ($Employeer in $PayloadData)
    {        $ID = $Employeer.EmployeeID
        IF (($Employeer.ASSIGNMENT_START_DATE).ToString("yyyy-MM-dd") -ge (Get-Date).AddDays(-5).ToString("yyyy-MM-dd"))
            {
            ...
            $Exist_AD_Account = get-aduser -Properties EmployeeID -filter {EmployeeID -eq $ID} -Server $GC 
                ...
                Else
                    {
                     ...
                     $SAMAcc = "$ConvertGivenName"+"."+"$ConvertSurName"
                        if ($SAMAcc.Length -gt 20) {$SAMAcc= $ConvertGivenName[0] + "." + $Convertfullinitials.Substring(0,$k) + "." + "$ConvertSurName";}
                     $UPN = $SAMAcc+"@rt.ru"
       
                        while (get-aduser -Properties userprincipalname -Filter {SamAccountName -eq $SAMAcc} -Server $GC)
                        #while (get-aduser -Properties userprincipalname -Filter {userprincipalname -eq $UPN} -Server $GC)
                              {
                ...
                #проверяем уникальность SamAccountName     
                     while (get-aduser -Properties userprincipalname -Filter {SamAccountName -eq $SAMAcc} -Server $GC)
                             ...
                             #if ($SAMAcc.Length -le 17) {
                             #$SAMAcc = $ConvertGivenName + "." + ($Convertfullinitials[0..$k] -join "") + "." + "$ConvertSurName";
                             ...
                             #$k++;}
                              ...
                              #}             
                                ...
                                try {
                                    ...
                                    {
                                    "Error caught $($Error[0].Exception)" |out-file $ErrorFolder\NotCreateMRFCAccount-$SAMAcc-$date.txt
                                    #Send-MailMessage -SmtpServer 'sce09ex001' -From 'NewEmployeersScript@rt.ru' -To 'fedorenko@center.rt.ru' -Subject "Warning - not create Account"`
                                    # -Attachments (Get-Content $ErrorFolder\NotCreateAccount-$SAMAcc-$date.txt) -Encoding "Unicode" -ea SilentlyContinue
                                    #Внимание! Если пользователь не может создаться, то. скорее всего такой уже есть в OU назначения. Блок catch прописывает недостающие атрибуты в УЗ

                                    Set-ADUser -identity "CN=$Name,$OU" -Replace @{EmployeeID=$ID} -Server $DC;
                                    Set-ADUser -identity $SAMAcc -Replace @{EmployeeType=$EmployeeType;EmployeeNumber=$EmployeeNumber} -Server $DC;
                                    Set-ADUser -identity "CN=$Name,$OU" -Replace @{extensionAttribute7="Write EmployeeID with new script for future Employeers"} -Server $DC;
                                ...
                                Set-ADUser -identity $SAMAcc -Add @{extensionAttribute8="New Script for future Employeers";EmployeeType=$EmployeeType;EmployeeNumber=$EmployeeNumber;extensionAttribute11=$Address} -Server $DC;
                                try {
                                    Enable-Mailbox -Identity $SAMAcc -Database $dbname -Alias $SAMAcc -DomainController $DC -Force 
                                    }
                                catch 
                                    {
                                    "Error caught $($Error[0].Exception)" |out-file $ErrorFolder\NotCreateMRFCMailbox-$SAMAcc-$date.txt -Append
                                    }
                                
                                # перемещение УЗ в региональное OU
                                try{    
                                    switch -Wildcard ($Address){
                                    "*Москва*" {Write-Host "It's a Moscow Address $($Address)"}
                                    #"*Краснодар*" {write-host "It's Krasnodar"}
                                    "*Московская*" {Write-Host "It's a Moscow Address $($Address)"}
                                    "*Костром*" {$DN=(Get-ADUser $SAMAcc -Server $DC).DistinguishedName;Move-ADObject -Identity $DN -TargetPath 'OU=Users,OU=KO,OU=MRF Center,DC=ce,DC=RT,DC=RU' -Server $DC -Confirm:$false;$DestPathLogsMRFC ='\\Mt.rt.ru\public\IT\New accounts\Logs\MRFC\KO'}
                                    "*Ярослав*" {$DN=(Get-ADUser $SAMAcc -Server $DC).DistinguishedName;Move-ADObject -Identity $DN -TargetPath 'OU=Users,OU=YR,OU=MRF Center,DC=ce,DC=RT,DC=RU' -Server $DC -Confirm:$false;$DestPathLogsMRFC ='\\Mt.rt.ru\public\IT\New accounts\Logs\MRFC\YR'}
                                    "*Калу*" {$DN=(Get-ADUser $SAMAcc -Server $DC).DistinguishedName;Move-ADObject -Identity $DN -TargetPath 'OU=Users,OU=KL,OU=MRF Center,DC=ce,DC=RT,DC=RU' -Server $DC -Confirm:$false;$DestPathLogsMRFC ='\\Mt.rt.ru\public\IT\New accounts\Logs\MRFC\KL'}
                                    "*Курск*" {$DN=(Get-ADUser $SAMAcc -Server $DC).DistinguishedName;Move-ADObject -Identity $DN -TargetPath 'OU=Users,OU=KR,OU=MRF Center,DC=ce,DC=RT,DC=RU' -Server $DC -Confirm:$false;$DestPathLogsMRFC ='\\Mt.rt.ru\public\IT\New accounts\Logs\MRFC\KR'}
                                    "*Иванов*" {$DN=(Get-ADUser $SAMAcc -Server $DC).DistinguishedName;Move-ADObject -Identity $DN -TargetPath 'OU=Users,OU=IV,OU=MRF Center,DC=ce,DC=RT,DC=RU' -Server $DC -Confirm:$false;$DestPathLogsMRFC ='\\Mt.rt.ru\public\IT\New accounts\Logs\MRFC\IV'}
                                    "*Белгород*" {$DN=(Get-ADUser $SAMAcc -Server $DC).DistinguishedName;Move-ADObject -Identity $DN -TargetPath 'OU=Users,OU=BL,OU=MRF Center,DC=ce,DC=RT,DC=RU' -Server $DC -Confirm:$false;$DestPathLogsMRFC ='\\Mt.rt.ru\public\IT\New accounts\Logs\MRFC\BL'}
                                    "*Липец*" {$DN=(Get-ADUser $SAMAcc -Server $DC).DistinguishedName;Move-ADObject -Identity $DN -TargetPath 'OU=Users,OU=LP,OU=MRF Center,DC=ce,DC=RT,DC=RU' -Server $DC -Confirm:$false;$DestPathLogsMRFC ='\\Mt.rt.ru\public\IT\New accounts\Logs\MRFC\LP'}
                                    "*Орел*" {$DN=(Get-ADUser $SAMAcc -Server $DC).DistinguishedName;Move-ADObject -Identity $DN -TargetPath 'OU=Users,OU=OR,OU=MRF Center,DC=ce,DC=RT,DC=RU' -Server $DC -Confirm:$false;$DestPathLogsMRFC ='\\Mt.rt.ru\public\IT\New accounts\Logs\MRFC\OR'}
                                    "*Рязан*" {$DN=(Get-ADUser $SAMAcc -Server $DC).DistinguishedName;Move-ADObject -Identity $DN -TargetPath 'OU=Users,OU=RZ,OU=MRF Center,DC=ce,DC=RT,DC=RU' -Server $DC -Confirm:$false;$DestPathLogsMRFC ='\\Mt.rt.ru\public\IT\New accounts\Logs\MRFC\RZ'}
                                    "*Смоленск*" {$DN=(Get-ADUser $SAMAcc -Server $DC).DistinguishedName;Move-ADObject -Identity $DN -TargetPath 'OU=Users,OU=SM,OU=MRF Center,DC=ce,DC=RT,DC=RU' -Server $DC -Confirm:$false;$DestPathLogsMRFC ='\\Mt.rt.ru\public\IT\New accounts\Logs\MRFC\SM'}
                                    "*Тул*" {$DN=(Get-ADUser $SAMAcc -Server $DC).DistinguishedName;Move-ADObject -Identity $DN -TargetPath 'OU=Users,OU=TL,OU=MRF Center,DC=ce,DC=RT,DC=RU' -Server $DC -Confirm:$false;$DestPathLogsMRFC ='\\Mt.rt.ru\public\IT\New accounts\Logs\MRFC\TL'}
                                    "*Тамбов*" {$DN=(Get-ADUser $SAMAcc -Server $DC).DistinguishedName;Move-ADObject -Identity $DN -TargetPath 'OU=Users,OU=TM,OU=MRF Center,DC=ce,DC=RT,DC=RU' -Server $DC -Confirm:$false;$DestPathLogsMRFC ='\\Mt.rt.ru\public\IT\New accounts\Logs\MRFC\TM'}
                                    "*Твер*" {$DN=(Get-ADUser $SAMAcc -Server $DC).DistinguishedName;Move-ADObject -Identity $DN -TargetPath 'OU=Users,OU=TR,OU=MRF Center,DC=ce,DC=RT,DC=RU' -Server $DC -Confirm:$false;$DestPathLogsMRFC ='\\Mt.rt.ru\public\IT\New accounts\Logs\MRFC\TR'}
                                    "*Владимир*" {$DN=(Get-ADUser $SAMAcc -Server $DC).DistinguishedName;Move-ADObject -Identity $DN -TargetPath 'OU=Users,OU=VL,OU=MRF Center,DC=ce,DC=RT,DC=RU' -Server $DC -Confirm:$false;$DestPathLogsMRFC ='\\Mt.rt.ru\public\IT\New accounts\Logs\MRFC\VL'}
                                    "*Воронеж*" {$DN=(Get-ADUser $SAMAcc -Server $DC).DistinguishedName;Move-ADObject -Identity $DN -TargetPath 'OU=Users,OU=VR,OU=MRF Center,DC=ce,DC=RT,DC=RU' -Server $DC -Confirm:$false;$DestPathLogsMRFC ='\\Mt.rt.ru\public\IT\New accounts\Logs\MRFC\VR'}
                                ...
                                $info.Адрес = $Address; 
                                $info | out-file "$DestPathLogsMRFC\$name $date.txt" utf8 
...
#$CEOU = 'OU=Users,OU=DR,OU=MRF Center,DC=ce,DC=RT,DC=RU'
$RRSOU = 'OU=Users,OU=RRS,DC=ce,DC=RT,DC=RU'
$RTIOU = 'OU=Users,OU=RTI,DC=ce,DC=RT,DC=RU'
$NewAccounts = Get-ADUser -SearchBase $OU -Properties EmployeeID,EmployeeType,lastlogondate,company,homeMDB -Filter * -Server $DC |`
where {($_.employeeid -ne $null) -and ($_.employeeType -ne $null) -and ($_.company -ne $null) -and ($_.homeMDB -ne $null) -and ($_.lastlogondate -ne $null)} 
foreach ($acc in $NewAccounts)
        {
        #if ($acc.company -match "Макрорегиональный филиал `"Центр`" ПАО `"Ростелеком`"") {Move-ADObject -Identity $acc -TargetPath $CEOU -Server $DC -Confirm:$false}
        if ($acc.company -match "ООО `"Ростелеком - Розничные системы`"") {Move-ADObject -Identity $acc -TargetPath $RRSOU -Server $DC -Confirm:$false}
        elseif ($acc.company -match "ООО `"Ростелеком-Интеграция`"") {Move-ADObject -Identity $acc -TargetPath $RTIOU -Server $DC -Confirm:$false}
        }
