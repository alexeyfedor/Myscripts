...
([company] like '%Корпоративный центр%' or ([company] like '%МФ ОЦО ПАО `"Ростелеком`"%' and ([extensionAttribute11] like '%г. Москва%' or [extensionAttribute11] like '%Московская обл.%' ))) `
        ...
        IF (($Employeer.Hire_Date).ToString("yyyy-MM-dd") -eq (Get-Date).ToString("yyyy-MM-dd"))
            ...
            try {
                ...
                #create set of attrubutes
                     ...
                     switch ($ChiefMail) {
                            'Oseevskiy@rt.ru' {$ChiefMail = 'Anna.Umanskaya@rt.ru '}
                            'Vladimir.Kirienko@rt.ru' {$ChiefMail = 'Darya.Shuvalova@rt.ru '}
                            'Aleksandr.Abramkov@RT.RU' {$ChiefMail = 'Marina.Bruskova@RT.RU'}
                            'a.levakov@center.rt.ru' {$ChiefMail = '111@center.rt.ru'}
                            '' {$ChiefMail = '111@center.rt.ru'}
                            #'fedorenko@center.rt.ru' {$ChiefMail = 'noreply@center.rt.ru'}
...
"@ 
            ...
            {
            "for User $Nameforexception  with EmployeeID $ID something went wrong. Check html output in $DestFolderHTML and set-aduser block in script " |out-file "$DestFolderError\$Nameforexception.txt" -Force utf8
            }
