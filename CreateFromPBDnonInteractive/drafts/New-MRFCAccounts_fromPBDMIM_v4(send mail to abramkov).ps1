...
$abramkov_Chief_path = 'C:\WorkSet\Abramkov_chief'
                        ...
                        #while (get-aduser -Properties userprincipalname -Filter {userprincipalname -eq $UPN} -Server $GC)
                              {
                               ...
                               if ([string]$($Employeer.mail_chif) -like 'Aleksandr.Abramkov@RT.RU') 
                                    {
                                    #write-host "it's a  real chief $($Employeer.mail_chif)"
                                    ...
                                    $altrecipients ='n_tiplyakova@center.rt.ru'
                                    $messageSubject = "Внимание. Новый работник - $Name"
                                    $info | ConvertTo-Html -as list -Head $Header |out-file "$abramkov_Chief_path\$name.htm" 
                                    $content = Get-Content "$abramkov_Chief_path\$name.htm"
$body=@"
Добрый день!<br>
В штат принимается работник, находящийся в прямом подчинении Вице-Президента - Директора макрорегионального филиала "Центр"<br>
<br>
$content

"@
                                    Send-MailMessage -SmtpServer $smtpserver -From $smtpfrom -To $recipient -cc $altrecipients -Encoding "Unicode" `
                                    -Subject $messageSubject -Body $body -BodyAsHtml -ErrorAction SilentlyContinue
                                    }                                 
                               #Miroslav.Voroshilov@rt.ru 
                               #Aleksandr.Abramkov@RT.RU
                    }#EO Else
