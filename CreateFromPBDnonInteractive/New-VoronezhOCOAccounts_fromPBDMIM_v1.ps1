#добавлен поиск по атрибуту ASSIGNMENT_START_DATE вместо Hire_date
...
and ([extensionAttribute11] like '%Воронеж%' or [extensionAttribute11] like '%ярослав%' )`
...
 
#$DataSet.Tables[0] | out-file "C:\temp2\OCOMRFC.csv"
$PayloadData = $DataSet.Tables[0]

#---------------------------------------------------------------------
#Variable Set
$ConnectionExchange = 'http://scedr07cas02.ce.rt.ru/PowerShell'
$GC = 'scedr08dc001.ce.rt.ru:3268'
$DC = 'scedr08dc001.ce.rt.ru'
$dbname= 'CEOCO_MAIN05','CEOCO_MAIN02','CEOCO_MAIN04' |Get-Random
$OU = 'OU=New_Accounts,OU=Users,OU=OCO,OU=MRF Center,DC=ce,DC=RT,DC=RU'
$DestPathLogsMRFCOCO = '\\Mt.rt.ru\public\IT\New accounts\Logs\MRFC_OCO'
$tempfolder = 'C:\temp\MRFCOCO'
        ...
        IF (($Employeer.ASSIGNMENT_START_DATE).ToString("yyyy-MM-dd") -ge (Get-Date).AddDays(-5).ToString("yyyy-MM-dd"))
            ...
            $Exist_AD_Account = get-aduser -Properties EmployeeID -filter {EmployeeID -eq $ID} -Server $GC 
                IF ($Exist_AD_Account) 
                    ...
                    $exist|out-file "$tempfolder\Exist_MRFC_OCO_Users_$date.csv" -Append utf8
                    }
                Else
                    {
                             ...
                             #$k++;}
                              ...
                              #}             
                                ...
                                try {
                                    ...
                                    {
                                    "Error caught $($Error[0].Exception)" |out-file $ErrorFolder\NotCreateVoronezhOCOAccount-$SAMAcc-$date.txt
                                ...
                                try {
                                    ...
                                    {
                                    "Error caught $($Error[0].Exception)" |out-file $ErrorFolder\NotCreateVoronezhOCOMailbox-$SAMAcc-$date.txt -Append
                                    }
                                ...
                                #$Create |out-file "$DestPathLogsMRFCOCO\$name $date.csv" -Append utf8
                                ...
                                $info.Адрес = $Address; 
                                $info | out-file "$DestPathLogsMRFCOCO\$name $date.txt" utf8 
