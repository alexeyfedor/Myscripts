...
#Основной Скрипт. Делаю через Invoke-Command -ComputerName 'vm-co-adm-01' -ScriptBlock $Script. Но подумай про double hop и использование CredSSP
...
#----------------------------------------------------------------------------
#Функция dot-sourcing write-log.Замени на import-module. Или сделай автозагружаемый модуль psm1,psd1
#. C:\scripts\Log_Rotation\Write-log.ps1
#Import-Module C:\scripts\Log_Rotation\Write-log.ps1;
Write-Log -Message "Starting log" -Path "$logdir\$env:computername\$env:computername.log"
#----------------------------------------------------------------------------
...
#else {
Send-MailMessage -SmtpServer $smtpserver -To $smtpto -From $smtpfrom -cc $copyto -Encoding "Unicode" -Subject $messageSubject -Body $body  -BodyAsHtml  -ErrorAction SilentlyContinue 
#}
...
#Invoke-Command -ComputerName 'vm-co-adm-01' -ScriptBlock $Script 

Invoke-Command -ComputerName 'vm-co-fs-01' -ScriptBlock $Script -Authentication CredSSP -Credential $cred
