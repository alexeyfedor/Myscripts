...
#$Global:SourceDir = "$($in.SourceData)" # не нужна
    ...
    foreach ($service in $services) 
        ...
        $servicename = $($service -split "=")[1]
            if (Get-Service $servicename -ErrorAction SilentlyContinue){
                try {
                Write-Log -Message "Trying to stop $servicename" -Path "$logdir\$env:computername\$env:computername.log"
                &net stop $servicename
                Write-Log -Message "$servicename is stopped" -Path "$logdir\$env:computername\$env:computername.log"
                }
...
$processes = $in.processes | Get-Member -MemberType NoteProperty | Select -ExpandProperty definition
    Foreach ($process in $processes) 
        ...
        $processname = $($process -split "=")[1]
            if (Get-Process -Name $processname)
                {
                    try {
                    Write-Log -Message "Try to kill $processname" -Path "$logdir\$env:computername\$env:computername.log" -Level Warn
...
$SourceData = $in.SourceData |get-member -MemberType noteproperty| Select -ExpandProperty definition
    Foreach ($SourceEntity in $SourceData) {
        $SourceEntityName=$($SourceEntity -split "=")[1]
        $Compressdata = Get-ChildItem -Recurse -Path $SourceEntityName -Attributes !Directory -Filter *.log  | Where-Object -FilterScript {$_.LastWriteTime -lt (Get-Date).AddDays(-$compressdays)}
    ...
    Write-Log -Message "Compress and move files with $Compressdays days old " -Path "$logdir\$env:computername\$env:computername.log"
...

        {
            ...
            # не забудь переобозвать архив, посокольку могут попадаться файлы с одинаковым названием из нескольких сорсов.
            $zipfile = $Origname.Replace('.log','.zip') #creates the zipped filename
            #$zipfile = $($Origdirectory+"--"+$Origname).Replace('.log','.zip') #creates the zipped filename
    ...
    Foreach ($Commandset in $commandsets)
         {
            If ($commandset.switch -eq "yes") 
               {
                 try {
                        Invoke-Expression $($comandset.Command)
                     }
                 catch {Write-Log -Message "$($Error[0].Exception)" -Path "$logdir\$env:computername\$env:computername.log" -Level Error -ErrorAction Continue}
               }#EO If
                    else {continue;}
...
$services = $in.Services | Get-Member -MemberType NoteProperty | Select -ExpandProperty definition
    foreach ($service in $services) 
        ...
        $servicename = $($service -split "=")[1]
            if (Get-Service $servicename -ErrorAction SilentlyContinue){
                try {
                Write-Log -Message "Trying to start $servicename" -Path "$logdir\$env:computername\$env:computername.log"
                &net start $servicename
                Write-Log -Message "$servicename is started" -Path "$logdir\$env:computername\$env:computername.log"
                }
            catch {Write-Log -Message "$($Error[0].Exception)" -Path "$logdir\$env:computername\$env:computername.log" -Level Error}
