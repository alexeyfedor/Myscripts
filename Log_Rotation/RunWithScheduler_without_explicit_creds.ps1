...
Copy-Item -Path $scriptitem -ToSession $session -Destination "$targetscriptfolder\draft_for_scheduler_v9.ps1"  -Force
...
$null = schtasks.exe /create /S  $hostname1 /RU exportcenter\adm_fedorenko /RP $Using:Pass  /TN $taskName /ST $time /SC DAILY /SD $startDate /RL HIGHEST /TR "powershell.exe & `"C:\scripts\draft_for_scheduler_v9.ps1;exit`""
