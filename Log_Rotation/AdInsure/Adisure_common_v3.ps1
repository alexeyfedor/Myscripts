...
$new_date = (Get-Date -Format 'yyyy-MM-dd_hh:mm:ss');
$new_date2 = (Get-Date -Format 'yyyy-MM-dd');
    ...
    foreach ($service in $services) 
        {
            ...
            if (Get-Service $service -ErrorAction SilentlyContinue){
                ...
                #&net stop $service
                get-wmiobject win32_service -filter "name=`'$service`'" | Invoke-WmiMethod -Name StopService;Start-Sleep 15; 
                Stop-PendingService
                $Service1 =Get-Service -Name $service
                $service1.WaitForStatus('Stopped')
...
#Import-Module "C:\Windows\System32\WindowsPowerShell\v1.0\Modules\microsoft.powershell.archive.1.2.2\Microsoft.PowerShell.Archive.psm1"
...
$compressionLevel= [System.IO.Compression.CompressionLevel]::Optimal
    ...
    Foreach ($SourceEntity in $SourceData) {
        ...
        $Compressdata = Get-ChildItem -Recurse -Path $SourceEntity -Attributes !Directory -Include "*.log*","*.txt"  | Where-Object -FilterScript {$_.LastWriteTime -lt (Get-Date).AddDays(-$compressdays)}
        Foreach ($origfile in $Compressdata)
            {
                # если архивируем данные из папки w3svc (это IIS), тогда в имена архивов будет включена дата. Нужно для того, чтобы впоследствии удалялись старые архивы. для не IIS такая манипуляция не нужна и там другая настройка (блок ELSE) 
                IF ($origfile.DirectoryName -match "w3svc") 
                    {
                      ...
                      #try {  
                        ...
                        $ZIPDirectoryNameDate = $ZIPDirectoryName+"-"+$new_date2
                        #$ZipFileName = $OrigFileName.Replace('.log','.zip')
                        Write-log -Message "Архивируем $($origfile.Name) из папки $ParentDirectoryName в архив $($ZIPDirectoryNameDate+".zip")" -Path "$logdir\$env:computername\$env:computername.log"
                        ...
                        [System.IO.Compression.ZipFile]::CreateFromDirectory($ParentDirectoryName, "$ZIPDirectoryNameDate.zip",$compressionLevel,$ExcludeBaseDirectory)
                        ...
                        #  }#EO Try
                        #catch {Write-log -Message "$($Error[0].Exception.Message)" -Path "$logdir\$env:computername\$env:computername.log" -Level Error}
                ...
                ELSE 
                    {
                      #try {
                        ...
                        $LastWriteTime = $origfile.LastWriteTime;
                        $ZIPDirectoryName = $archDir+(Split-path $ParentDirectoryName -NoQualifier)
                        Write-log -Message "Архивируем $($origfile.Name) из папки $ParentDirectoryName в архив $($ZIPDirectoryName+".zip")" -Path "$logdir\$env:computername\$env:computername.log"
                        $ZIPParentDirectoryName = Split-Path $ZIPDirectoryName -Parent
                           if (-not(Test-Path $ZIPParentDirectoryName)) {New-Item -Path $ZIPParentDirectoryName -ItemType Directory}
...
#----------------------------------------------------------------------------
#Блок удаления старых пустых папок. Нужно только для AdInsure.
$folders = Get-ChildItem $SourceData -Attributes Directory -Recurse |where {$_.name -match ((get-date).ToString("yyyyMM"))+"\d{2}"}
    ...
    If ((Get-ChildItem -Path $folder.FullName -Recurse).count -eq "0") 
        {
            ...
            Write-Log -Message "Удаляем пустую папку  $($folder.FullName)" -Path "$logdir\$env:computername\$env:computername.log" -Level Warn
