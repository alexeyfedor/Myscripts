...
$Data = [ordered]@{
    ...
    DestinationData = 'D:\Archive'
    LogsPath = "\\VM-CO-ADM-01\RotationTasks\Logs\"
    DateTime = @{
        CompressAndMoveDataDays = '2'
    ...
    Recipients = @{
        Recipient1 = 'ins_support@exportcenter.ru'
        Recipient2 = 'shmarov@exportcenter.ru'
    }
    
  

    }|ConvertTo-Json |Set-Content $Path -Force   #для  posh v4 нужно использовать ключ -compress. Иначе при конвертации появляется ошибка. Но в этом случае ini файл становится трудно читаеммым. Альренативный вариант - ставить posh 5.1, или делать конфигрурационный файл на хосте сposh 5.1
