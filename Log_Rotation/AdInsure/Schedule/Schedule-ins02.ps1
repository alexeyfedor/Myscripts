...
$scriptitem = get-Item "C:\scripts\Log_Rotation\AdInsure\AdInsure-ins02-common.ps1"
$session = New-PSSession -ComputerName $comp;
$targetscriptfolder = 'c:\scripts';
$Moduletargetscriptfolder ='C:\Windows\System32\WindowsPowerShell\v1.0\Modules\microsoft.powershell.archive.1.2.2'

Copy-Item -Path $scriptitem -ToSession $session -Destination "$targetscriptfolder\AdInsure-ins02-common.ps1"  -Force
...
$null = schtasks.exe /create /S  $hostname1 /RU exportcenter\adm_fedorenko /RP $Using:Pass  /TN $taskName /ST $time /SC DAILY /SD $startDate /RL HIGHEST /TR "powershell.exe & `"C:\scripts\AdInsure-ins02-common.ps1;exit`""
