...
$scriptitem = get-Item "C:\scripts\Log_Rotation\AdInsure\AdInsure-ins01-common.ps1"
$session = New-PSSession -ComputerName $comp;
$targetscriptfolder = 'c:\scripts';
$Moduletargetscriptfolder ='C:\Windows\System32\WindowsPowerShell\v1.0\Modules\microsoft.powershell.archive.1.2.2'

Copy-Item -Path $scriptitem -ToSession $session -Destination "$targetscriptfolder\AdInsure-ins01-common.ps1"  -Force
...
#Read-Host -Prompt "Введите пароль" -AsSecureString |ConvertFrom-SecureString | Set-Content C:\windows\temp\p3.txt
...
#$UserName = "exportcenter\REC_Service_Script"
...
# Создаем задачу в 
$null = schtasks.exe /create /S  $hostname1 /RU exportcenter\adm_fedorenko /RP $Using:Pass  /TN $taskName /ST $time /SC DAILY /SD $startDate /RL HIGHEST /TR "powershell.exe & `"C:\scripts\AdInsure-ins01-common.ps1;exit`""
#$null = schtasks.exe /create /S  $hostname1 /RU exportcenter\REC_Service_Script /RP $Using:Pass  /TN $taskName /ST $time /SC DAILY /SD $startDate /RL HIGHEST /TR "powershell.exe & `"C:\scripts\AdInsure-ins01-common.ps1;exit`""
