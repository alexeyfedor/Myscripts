...
# Присваиваем переменной $Name значение из поля ФИО
    ...
    $OUTPUT= [System.Windows.Forms.MessageBox]::Show("Найден пользователь $($global:existAduser.name).`nEmail $($global:existAduser.mail).`nEmployeeID $($global:existAduser.employeeid).`Разблокировать УЗ?" , "Status" , 4) 
        if ($OUTPUT -eq "YES" ) 
        {
        #создание учетной записи
        $global:existAduser | Enable-User; [System.Windows.Forms.MessageBox]::Show("УЗ пользователя $($global:existAduser.name) разблокирована.`n`n"); break; 
        if ($OUTPUT -eq "NO" ) 
