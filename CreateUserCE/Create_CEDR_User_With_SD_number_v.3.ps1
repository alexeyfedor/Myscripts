...
Function Create-ADUser {
    ...
    process{
     ...
     if(("$translit_imya"+"."+"$translit_familia").Length -le 20) {$mailnickname = "$translit_imya"+"."+"$translit_familia"}
     elseif(("$translit_imya"+"."+"$translit_familia").Length -gt 20) {$mailnickname = "$($translit_imya[0])"+"."+"$translit_familia"}
        ...
        $richtextboxStatus.Text += "Создаем пользователя $mailnickname `n"
        ...
        'UserPrincipalName' = "$($mailnickname+'@ce.rt.ru')"
        ...
        -or ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole("GCE_DR_AccountOperators")
...

            {
                ...
                Start-Sleep 5;
                Set-ADUser -identity $mailnickname -Add @{extensionAttribute13='DR'} -Server 'scedr07dc002.ce.rt.ru';
                #Set-ADUser -identity $SAMAccountNAme -Add @{extensionAttribute14='CE_Exch2010'} -Server 'scedr07dc002.ce.rt.ru';
                Set-ADUser -identity $mailnickname -Add @{extensionAttribute8="$($SD)"} -Server 'scedr07dc002.ce.rt.ru';
                Set-ADUser -identity $mailnickname -Add @{extensionAttribute9="$($Account_owner)"} -Server 'scedr07dc002.ce.rt.ru';
               ...
               $richtextboxStatus.Text += "Добавляем  E-mail пользователя $($mailnickname+'@rt.ru') `n"
               Enable-Mailbox -Identity $mailnickname -Alias $mailnickname -Database $dbname -DomainController 'scedr07dc002.ce.rt.ru'
               $richtextboxStatus.Text += "Пользователь создан в организационном подразделении `n$OU`nЕго данные:`nЛогин:$mailnickname`nДомен:ce.rt.ru`nПароль:$userpass`nEmail:$($mailnickname+'@rt.ru')`n`n`n" 
                            
                    
                    $LOGoutput= "Заявка № $SD. Огранизационное подразделение $OU`. Логин: $mailnickname. Домен:ce.rt.ru .Пароль:$userpass. Email:$($mailnickname+'@rt.ru').Учетная запись создана оператором $Account_owner";
...
if ((Get-Module -ListAvailable | where { $_.Name -eq 'ActiveDirectory'}) -eq $null) {
		...
		If ((Get-Module ActiveDirectory) -eq $null) {
			...
			$richtextboxStatus.Text += "Импортирую модуль ActiveDirectory`n"
		}
		else {
