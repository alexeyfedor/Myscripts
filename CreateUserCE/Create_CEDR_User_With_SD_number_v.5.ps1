...
Function Create-ADUser {
    ...
    process{
        ...
        if(("$translit_imya"+"."+"$translit_familia").Length -le 18)
            {
         ...
         elseif(("$translit_imya"+"."+"$translit_familia").Length -gt 18)
            {

            $SAMAcc = "$($translit_imya[0])"+"."+"$translit_familia";
            $UPN = "$($translit_imya[0])"+"."+"$translit_familia"+"@rt.ru"
            $n=1
            while ((get-aduser -Properties userprincipalname -Filter {userprincipalname -eq $UPN} -Server "scedr07dc002.ce.rt.ru:3268") -ne $null)
                {
