...
Function Create-ADUser {
    ...
    process{
        ...
        $ExchangeSession = New-PSSession -ConfigurationName Microsoft.Exchange -ConnectionUri http://sce09ex01.ce.rt.ru/PowerShell
        Import-PSSession $ExchangeSession -AllowClobber
        #проверка уникальности UPN и количества символов для SamAccountName (ограничение в 20 символов, однако выбрано 18 на случай, если будет найдет дублирующий UPN и для создаваемого придется добавить точку и первую букву отчества)
        #если SamAccountName короткий (меньше 18 символов)
         $k = 1;$n=1; 
       $SAMAcc = "$translit_imya"+"."+"$translit_familia"
       if ($SAMAcc.Length -gt 20) {$SAMAcc= $translit_imya[0] + "." + $translit_otch.Substring(0,$k) + "." + "$translit_familia";}

       #$UPN = "$translit_imya"+"."+"$translit_familia"+"@rt.ru"
       $UPN = $SAMAcc+"@rt.ru"
       
        while (get-aduser -Properties userprincipalname -Filter {SamAccountName -eq $SAMAcc} -Server "scedr07dc002.ce.rt.ru:3268")

            {
                
                if ($SAMAcc.Length -lt 19)
                    {
                    $SAMAcc= $translit_imya + "." + $translit_otch.Substring(0,$k) + "." + "$translit_familia";
                    $SAMAcc = $SAMAcc.substring(0, [System.Math]::Min(20,$SAMAcc.Length));
                    #$UPN = $translit_imya + "." + $translit_otch.Substring(0,$k) + "." + "$translit_familia" + "@rt.ru";
                    $UPN = $SAMAcc + "@rt.ru";
                ...
                elseif ($SAMAcc.Length -ge 19)
                    {
                    $SAMAcc= $translit_imya[0] + "." + $translit_otch.Substring(0,$n) + "." + "$translit_familia"
                    $SAMAcc = $SAMAcc.substring(0, [System.Math]::Min(20,$SAMAcc.Length));
        ...
        -or ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole("GCE_DR_AccountOperators")
...

            {
                ...
                Set-ADAccountControl -identity $SAMAcc -PasswordNotRequired $false -Server 'scedr07dc002.ce.rt.ru' ;
                Start-Sleep 2;
                #ADD-ADGroupMember “GCE_bcuser_proxyonly” –members $SAMAcc -Server 'scedr07dc002.ce.rt.ru'
                ...
                #$DBName = (Get-MailboxDatabase | where {$_.name -match "ce-min*"}).name|Get-Random #|Out-String;
